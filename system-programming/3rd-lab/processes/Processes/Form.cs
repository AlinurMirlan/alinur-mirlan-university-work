    ...
    public partial class ProcessesTracker : Form
    {
        ...
        private void AddProcessButton_Click(object sender, EventArgs e)
        {
            ...
            foreach (string fileName in fileNames)
            {
                ...
                Task.Run(() => LogState(process));

                int index = dataGridView.Rows.Add(process.ProcessName, process.RamUsage(), process.TotalProcessorTime, process.Threads.Count, process.HandleCount, process.Id);
        ...
        private void UpdateGridView()
        {
            for (int i = 0; i < _activeProcesses.Count; i++)
            {
                ...
                {
                    process.Refresh();
                    dataGridView.Rows[i].SetValues(process.ProcessName, process.RamUsage(), process.TotalProcessorTime, process.Threads.Count, process.HandleCount, process.Id);
                }
            }
        }

        private void LogState(Process process)
        {
            int ticks = 0;
            ...
            do
            {
                ...
                if (process.RamUsage() >= _processLimit!.MemoryUsageLimit)
                {
                ...
                else if (process.RamUsage() >= _dangerousThreshold.MemoryUsageLimit)
                {
